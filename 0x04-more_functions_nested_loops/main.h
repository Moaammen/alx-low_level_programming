/**
 * _putchar: function print single char
 */
int _putchar(char c);
/**
 * _isupper: function check for uppercase char
 */
int _isupper(int c);
/**
 * _isdigit: function check for digit
 */
int _isdigit(int c);
/**
 * mul: function multiplies two integers
 */
int mul(int a, int b);
/**
 * print_numbers: function print for 0 to 9 with _putchar
 */
void print_numbers(void);
/**
 * pritn_most_numbers: function print numbers from 0 to 9 avoid printing 2&4
 */
void print_most_numbers(void);
/**
 * more_numbers: function print from 0 to 14 with _putchar
 */
void more_numbers(void);
/**
 * print_line: function that print _ 
 */
void print_line(int n);
/**
 * print_diagonal: function print diagonal line
 */
void print_diagonal(int n);
/**
 * print_square: function print square
 */
void print_square(int size);
/**
 * print triangle: function print triangle
 */
void print_triangle(int size);
/**
*print_number: function print integer using putchar
*/
void print_number(int n);
